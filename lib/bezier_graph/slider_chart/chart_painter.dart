...
class ChartPainter extends CustomPainter {
  ...
  //注意这里是左下开始 右上结束。
  double startX = 0, endX = 0, startY = 0, endY = 0; //表的实际绘制区域（去除padding）
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // canvas.drawPoints(PointMode.polygon, pointOffsetList, Paint()..strokeWidth = 5);
    // pointOffsetList.map((e) => canvas.drawCircle(e, 5, Paint()..style = PaintingStyle.fill..color = Colors.red..strokeWidth = 1));
    pointOffsetList.forEach((element) {
      canvas.drawCircle(
        element,
        5,
        Paint()
  ...
  void _drawLine(Canvas canvas, Size size) {
    ...
    // var length = list.length.toInt() - (list.length.toInt() * leftStart) - (list.length.toInt() * (1 - rightEnd));
    ...
    //   var extractPath = list[i].extractPath(startExtr, endExtr, startWithMoveTo: true);
    //   //extractPath.getBounds()
    //   linePath.addPath(extractPath, Offset(0, 0));
    //   shadowPath = extractPath;
    // }

    Path shadowPath = Path.from(_path!);

    ///给画笔添加着色器，可以通过各种渐变createShader（）方法生成着色器
    /// 这里使用的是线性渐变、还有RadialGradient,SweepGradient
    if (fillColors != null) {
      var shader = LinearGradient(
              begin: Alignment.topCenter, end: Alignment.bottomCenter, tileMode: TileMode.clamp, colors: fillColors)
      ...
      if (chartBeans != null && chartBeans!.length > 0 && minMax[0] > 0) {
        ...
        for (int i = 0; i < length; i++) {
          if (i == 0) {
            ...
            pointOffsetList.add(Offset(key, value));
            continue;
          }
          ...
          pointOffsetList.add(Offset(currentX, currentY));
